...
"""
    ...
    markup = types.InlineKeyboardMarkup(row_width=2)
    markup.add(
        types.InlineKeyboardButton("📞 اتصل الآن", url=f"tel:{+201030319293
        types.InlineKeyboardButton("📧 أرسل بريد", url=f"mailto:{company['a"aht-s.com
        types.InlineKeyboardButton("🌐 زور موقعنا", url=company['aht-s.com
...
{PROFESSIONAL_DATA['company']['+201030319293		

📧 أو عبر البريد الإلكتروني:
{PROFESSIONAL_DATA['company']['info"aht-s.com

✨ سنكون سعداء بمساعدتك!
"""
    elif lang == "ru":
        text = f"""
💬 *Для запроса об этом туре*email
